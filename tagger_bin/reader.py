...
class BinDataset(Dataset):
    ...
    def __init__(self, tsv_file,isTrain,pretrained_embeddings=False, word_to_ix={}, tag_to_ix={},event_to_ix={}):
        """
        Args:
        ...
        for i in range(bin_np.shape[0]):
            ...
            if bin_np[i][1] == None or i == bin_np.shape[0] - 1:  # append all docs including the last one
                if (i == bin_np.shape[0] - 1):  # append last line
                    ...
                    bin_tokens.extend(utils.prepare_sequence(utils.lstToString(utils.strToLst(bin_np[i][1])).split(),
                                                             self.word_to_ix))


                if (docNr != -1):
                    bin_tokens = np.asarray(bin_tokens)
                    ...
                    match.append([bin_tokens, tag_id,ec_id,event_duration_idx,independent_event_idx,event_type,event_id,match_name])
                ...
                #print (bin_tokens)
                bin_tokens.extend(
                    utils.prepare_sequence(utils.lstToString(utils.strToLst(bin_np[i][1])).split(), self.word_to_ix))
